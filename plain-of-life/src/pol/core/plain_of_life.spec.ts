...
import { type ExtPlainOfLife, PlainOfLife } from './plain_of_life'
...
import { type SerializablePlainOfLife } from './serializable_plain_of_life'
...
describe('Plain of life', () => {
  ...
  describe('getRulesName', () => {
    ...
    it('returns the rules name correctly and throws an exception if there is no rule name', () => {
      ...
      vi.spyOn(ruleNames, 'getName').mockReturnValue(expectedName)
  ...
  describe('executeTurn', () => {
    describe('if not game over (if there are alive cells)', () => {
      ...
      it('increases the current turn and indicates that game is not over', () => {
        expect(executeTurnResult).toBe(true)
        expect(plainOfLife.isGameOver).toBe(false)
        expect(plainOfLife.currentTurn).toBe(1n)
      })

      it('calls executeTurn of rules as expected', () => {
        expect(rules.executeTurn).toHaveBeenCalledExactlyOnceWith(plain, cellContainers as CellContainers<TestRules>, 1n)
      })
      ...
      it('updates the family tree', () => {
        expect(familyTree.update).toHaveBeenCalledExactlyOnceWith(
    ...
    describe('if game over (if there are no alive cells left)', () => {
      ...
      it('does not increases the current turn and indicates that game is over', () => {
        expect(executeTurnResult).toBe(false)
        expect(plainOfLife.isGameOver).toBe(true)
  ...
  describe('createFromSerializable() with not registered rule constructor', () => {
    ...
    it('throws an error', () => {
      vi.spyOn(cellNames, 'getConstructor').mockReturnValue(TestCell)
  ...
  function createPlainOfLifeAndExecuteTurn(enforceGameOver: boolean): void {
    ...
    vi.spyOn(rules, 'executeTurn')
    vi.spyOn(familyTree, 'update')
  ...
  function plainOfLifeToSerializable() {
    ...
    vi.spyOn(ruleNames, 'getName').mockReturnValue('TestRules')
    vi.spyOn(cellNames, 'getName').mockReturnValue('TestCell')
  ...
  function plainOfLifeFromSerializable() {
    ...
    vi.spyOn(ruleNames, 'getConstructor').mockReturnValue(TestRules)
    vi.spyOn(cellNames, 'getConstructor').mockReturnValue(TestCell)
