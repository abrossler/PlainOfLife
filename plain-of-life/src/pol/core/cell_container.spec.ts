...
import { type SerializableCellContainer } from './serializable_plain_of_life'
...
describe('Cell Container', () => {
  ...
  describe('construction', () => {
    it('fills properties correctly', () => {
      expect(cellContainer.isDead).toBe(false)
    ...
    it('creates a correct cyclic list of one cell container', () => {
      expect(cellContainer.next === cellContainer).toBe(true)
      expect((cellContainer as any)._prev === cellContainer).toBe(true)
    })
  ...
  describe('die', () => {
    ...
    it('marks the died container as dead', () => {
      expect(child2Container.isDead).toBe(true)
    })
  ...
  describe('initFromSerializable', () => {
    it('throws an error if cell class is not registered', () => {
      vi.spyOn(cellNames, 'getName').mockReturnValue('TestCell') // Spy for toSerializable
  ...
  describe('initFromSerializable', () => {
    ...
    it('sets up the list of all alive cells correctly', () => {
      ...
      expect(fromSerializable.isDead).toBe(false)
      expect(fromSerializable.next.isDead).toBe(false)
    })
